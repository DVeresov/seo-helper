...
Заметьте — я назвал это `Services`, а не `Models`. В Laravel модели обычно работают с **локальной** БД через Eloquent. Поскольку у вас внешний источник данных (MODX), правильнее оформить это как **Service класс**. Это чище архитектурно.

Начнём с `ModxApiClient`?
